...
import { 
  View, Text, StyleSheet, ActivityIndicator, TouchableOpacity, Alert, StatusBar, Platform, Modal, FlatList 
} from 'react-native';
...
const MetroMapScreen = () => {
  ...
  const [selectingType, setSelectingType] = useState<'FROM' | 'TO' | null>(null); // Để mở Modal chọn ga
  ...
  useEffect(() => {
   const startLocationTracking = async () => {
    ...
    if (status !== 'granted') {
    Alert.alert('Thông báo', 'Cần cấp quyền vị trí để tìm ga gần bạn nhất.');
    return;
    }
  ...
  const routeCoordinates = useMemo(() => 
    stations.filter(s => s.lat && s.lon).map(s => ({ latitude: s.lat, longitude: s.lon })), 
  [stations]);
  ...
  const updateHighlightPolyline = (from: any, to: any) => {
    ...
    const newCoordinates = stations
        .filter(s => s.order_index >= startIdx && s.order_index <= endIdx && s.lat && s.lon)
  ...
  return (
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.topPanelWrapper} edges={['top']}>
        <View style={styles.topCard}>
          ...
          <View style={styles.inputContainer}>
            <View style={{flex: 1}}>
              <TouchableOpacity style={styles.inputBox} onPress={() => setSelectingType('FROM')}>
              ...
              <View style={{height: 8}} />
              <TouchableOpacity style={styles.inputBox} onPress={() => setSelectingType('TO')}>
                <View style={styles.inputContent}>
            ...
            {/* Nút Đảo chiều */}
            <TouchableOpacity style={styles.swapBtn} onPress={handleSwapStations}>
              <MaterialCommunityIcons name="swap-vertical" size={24} color="#003eb3" />
            </TouchableOpacity>
          </View>
      ...
      {loading ? (
        ...
        >
          ...
          {stations.map((station) => {
            ...
            if (station.code === fromStation?.code) { markerColor = '#4CAF50'; isSelected = true; } // Ga đi: Xanh lá
            else if (station.code === toStation?.code) { markerColor = '#FF5722'; isSelected = true; } // Ga đến: Cam đỏ
            ...
            return (
              ...
              >
                <View style={styles.markerContainer}>
                  <View style={[
                    styles.markerDot, 
                    station.code === nearestStation?.code ? styles.nearestDot : {}, // Highlight ga gần nhất
                    station.order_index === 1 || station.order_index === stations.length ? styles.endPoint : {}
                  ]} />
                  <View style={[
                    styles.markerLabel,
                    station.code === nearestStation?.code ? styles.nearestLabel : {}
                  ]}>
                    <Text style={[
                      styles.markerText,
                      station.code === nearestStation?.code ? {color: 'white', fontWeight: 'bold'} : {}
                    ]}>{station.name}</Text>
                  </View>
                </View>
              </Marker>
      ...
      <View style={styles.controlsContainer}>
        {/* Nút tìm ga gần nhất */}
        {nearestStation && (
          <TouchableOpacity style={styles.nearestBtn} onPress={goToNearestStation}>
            <View>
        ...
        <TouchableOpacity style={styles.controlBtn} onPress={goToMyLocation}>
          <MaterialCommunityIcons name="crosshairs-gps" size={24} color="#003eb3" />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.footerLegend}>
        ...
        <View style={{flexDirection: 'row', alignItems: 'center', marginLeft: 15}}>
           <View style={[styles.legendDot, {backgroundColor: '#4CAF50', borderWidth: 2, borderColor: 'white'}]} />
                ...
                <FlatList 
                    ...
                    renderItem={({item}) => (
                        <TouchableOpacity style={styles.stationItem} onPress={() => handleSelectStation(item)}>
                            <MaterialCommunityIcons name="train" size={24} color="#003eb3" style={{marginRight: 10}} />
                            <View>
                                <Text style={styles.stationName}>{item.name}</Text>
                                {nearestStation?.code === item.code && (
                                    <Text style={{fontSize: 10, color: '#4CAF50'}}>📍 Gần bạn nhất</Text>
                                )}
                            </View>
                        </TouchableOpacity>
                    )}
...
const styles = StyleSheet.create({
  ...
  // Top Panel
  topPanelWrapper: { position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, padding: 16 },
  topCard: { backgroundColor: 'white', borderRadius: 16, padding: 15, shadowColor: '#000', shadowOpacity: 0.1, elevation: 5 },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  headerTitle: { fontSize: 16, fontWeight: 'bold', color: '#00235B' },
  
  inputContainer: { flexDirection: 'row', alignItems: 'center' },
  inputBox: { 
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#F5F7FA', justifyContent: 'space-between',
  ...
  buyBtn: { 
    flexDirection: 'row', backgroundColor: '#003eb3', padding: 12, borderRadius: 10, 
  ...
  markerDot: { width: 14, height: 14, borderRadius: 7, backgroundColor: '#fff', borderWidth: 3, borderColor: '#ff4d4f', marginBottom: 2 },
  endPoint: { borderColor: '#ff4d4f', width: 16, height: 16, borderRadius: 8 },
  nearestDot: { borderColor: '#4CAF50', width: 18, height: 18, borderRadius: 9, borderWidth: 4 }, // Xanh lá cho ga gần nhất
  
  markerLabel: { backgroundColor: 'rgba(255,255,255,0.9)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, borderWidth: 1, borderColor: '#eee', elevation: 2 },
  nearestLabel: { backgroundColor: '#4CAF50', borderColor: '#388E3C' },
  markerText: { fontSize: 10, fontWeight: '700', color: '#333' },

  // Controls
  controlsContainer: { position: 'absolute', right: 16, bottom: 100, alignItems: 'flex-end' },
  controlBtn: { width: 50, height: 50, borderRadius: 25, backgroundColor: 'white', justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.2, elevation: 5, marginTop: 10 },
  
  nearestBtn: { 
    flexDirection: 'row', alignItems: 'center', backgroundColor: 'white', 
    padding: 10, borderRadius: 12, shadowColor: '#000', shadowOpacity: 0.2, elevation: 5, marginBottom: 5 
  },
  nearestTitle: { fontSize: 10, color: '#666', fontWeight: '600' },
  nearestName: { fontSize: 12, color: '#003eb3', fontWeight: 'bold' },
  nearestIcon: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#4CAF50', justifyContent: 'center', alignItems: 'center', marginLeft: 10 },

  // Footer
  footerLegend: { position: 'absolute', bottom: 30, alignSelf: 'center', backgroundColor: 'white', padding: 12, borderRadius: 30, flexDirection: 'row', justifyContent: 'center', shadowColor: '#000', shadowOpacity: 0.1, elevation: 4 },
  ...
  stationItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 15, borderBottomWidth: 1, borderColor: '#eee' },
