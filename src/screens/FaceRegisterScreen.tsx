  ...
  const handleRegisterBiometric = async () => {
    try {
      ...
      if (result.success) {
        ...
        const res = await axiosClient.post<any, ApiResponse>('/auth/enable-biometric', { 
        ...
        if (res.success) {
          ...
          Alert.alert('Thành công', 'Tính năng đăng nhập nhanh đã được kích hoạt!', [
            { text: 'Xong', onPress: () => navigation.goBack() }
          ]);
        } else {
          Alert.alert('Lỗi', res.message || 'Không thể lưu thông tin vào hệ thống.');
        }
      }
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.desc}>
        Kích hoạt FaceID hoặc Vân tay để đăng nhập nhanh chóng mà không cần nhập mật khẩu.
      </Text>
      ...
      <TouchableOpacity 
        ...
        disabled={loading}
      >
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>BẮT ĐẦU THIẾT LẬP</Text>}
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  desc: { fontSize: 15, color: '#666', textAlign: 'center', marginTop: 15, lineHeight: 22 },
  btn: { backgroundColor: '#28a745', width: '100%', padding: 18, borderRadius: 15, alignItems: 'center', marginTop: 40 },
